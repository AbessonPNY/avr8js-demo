...
import { SIMON_CODE, PITCHES_H } from './code';
...
const led11 = document.querySelector<LEDElement>('wokwi-led[color=blue]');
...
function executeProgram(hex: string) {
  ...
  runner.portB.addListener(() => {
    ...
    led11.value = runner.portB.pinState(3) === PinState.High;
  });
...
async function compileAndRun() {
  ...
  try {
    statusLabel.textContent = 'Compiligation...';
    const result = await buildHex(editor.getModel().getValue(), [
      { name: 'pitches.h', content: PITCHES_H }, { name: 'toto.h', content: 'toto.h' }
    ]);
